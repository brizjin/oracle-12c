...
begin
	if :can_run_patch = 0 then
		...
		'Текущая версия ТЯ '||:lVersion||' не позволяет выполнить патч схемы-владельца IBSO!'||chr(10)||
		'Выполните полный апгрейд.';
	else
		:mess := 'Текущая версия ТЯ '||:lVersion||' позволяет выполнить патч схемы-владельца IBSO.';
	end if;
end;
/
