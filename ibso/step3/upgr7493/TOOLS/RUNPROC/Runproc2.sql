...
create or replace package body RUNPROC_PKG as
/*
 *  $HeadURL: http://hades.ftc.ru:7382/svn/pltm2/Core/Utils/RUNPROC/Runproc2.sql $
 *  $Author: Alexey $
 *  $Revision: 15078 $
...
function SendQuery(     pText   varchar2
                       ,pType   varchar2
                       ,pId     varchar2
                       ,pCode   varchar2 := null ) return number as
    vText   varchar2(2000);
    vRowid  rowid;
begin
    -- проверка длины кода
    ...
    begin
        -- вставим в таблицу
        insert into queries (type,text,id, username, created, code )
            values (pType, vText, pId, user, sysdate, pCode )
...
function  Put_Query(     pText   varchar2
                       ,pId     varchar2
                       ,pCode   varchar2 := null ) return number is
begin return SendQuery(pText,'B',pId,pCode); end;
------------------------------------------------------------------------
...
function AddQuery( pText   varchar2,
                   ...
                   pType   varchar2 default null) return number is
    vText   varchar2(2000);
...
begin
    ...
    begin
        -- вставим в таблицу
        insert into queries (type, text, id, username, created, code )
            values (vType, vText, queries_id.nextval, user, vTime, vCode )
...
begin
    if pBroken='1' then
        ...
        vCode := 'EXECUTE';
        if vFail>nvl(stdio.num_set('JOBS_MAX_FAILURES'),10) then
            vCode := 'EXECUTED';
        elsif vFail>0 then
...
begin
  vJob := pId;
  select text,created,rowid,failures into vText,vNext,vRowid,vFail
  ...
  exception when others then
    Rollback;
    vFail := nvl(vFail,0)+1;
    UpdQuery( pId, vNext, vBroken, true, vFail);
    commit;
    raise;
  end;
end;
