prompt xrc_xmldom
create or replace package xrc_xmldom as
/*
 *	$HeadURL: http://hades.ftc.ru:7382/svn/pltm2/CorePkg/tags/7.4.9.3/IBS/xmldom1.sql $
 *  $Author: Alexey $
 *  $Revision: 15072 $
...
/**
 * Extproc interface type
 */
SUBTYPE Handle IS raw(8);
 ...
 * DOM interface types
 */
TYPE DOMNode IS RECORD (ID Handle);
TYPE DOMNamedNodeMap IS RECORD (ID Handle);
TYPE DOMNodeList IS RECORD (ID Handle);
TYPE DOMAttr IS RECORD (ID Handle);
TYPE DOMCDataSection IS RECORD (ID Handle);
TYPE DOMCharacterData IS RECORD (ID Handle);
TYPE DOMComment IS RECORD (ID Handle);
TYPE DOMDocumentFragment IS RECORD (ID Handle);
TYPE DOMElement IS RECORD (ID Handle);
TYPE DOMEntity IS RECORD (ID Handle);
TYPE DOMEntityReference IS RECORD (ID Handle);
TYPE DOMNotation IS RECORD (ID Handle);
TYPE DOMProcessingInstruction IS RECORD (ID Handle);
TYPE DOMText IS RECORD (ID Handle);
TYPE DOMImplementation IS RECORD (ID Handle);
TYPE DOMDocumentType IS RECORD (ID Handle);
TYPE DOMDocument IS RECORD (ID Handle);
...
PROCEDURE getNodeValue(n DOMNode, cl IN OUT CLOB);
...
FUNCTION importNode(n DOMDocument, srcNode IN DOMNode, deep boolean) RETURN DOMNode;
FUNCTION adoptNode(n DOMDocument, srcNode IN DOMNode) RETURN DOMNode;
...
FUNCTION createTextNode(doc DOMDocument, data CLOB) RETURN DOMText;
...
/**
 * Writes XML document to specified file using the document character set
 * or UTF-8 if character was not specified for document
 */
PROCEDURE writeToFile(doc DOMDocument, fileName VARCHAR2);

/**
 * Writes XML document to specified buffer using the the document character set
 * or UTF-8 if character was not specified for document
 */
PROCEDURE writeToBuffer(doc DOMDocument, buffer IN OUT VARCHAR2);

/**
 * Writes XML document to specified clob using the the document character set
 * or UTF-8 if character was not specified for document
 */
